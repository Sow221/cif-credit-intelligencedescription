...
def test_lending_club_score_accepts_null_for_missing_numeric_feature():
    """Une valeur numérique inconnue doit être envoyable en JSON strict (null), jamais NaN
    (rejeté par tout client HTTP conforme — httpx refuse même de l'émettre)."""
    import numpy as np
    import pandas as pd
    ...
    with TestClient(app) as client:
        app.state.lending_club_predictor = stub_predictor
        payload = train[list(FEATURES)].iloc[0].to_dict()
        payload["mort_acc"] = None  # valeur numérique explicitement inconnue
