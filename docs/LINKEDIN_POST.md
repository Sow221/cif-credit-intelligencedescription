...
⚙️ Infrastructure as Code — **CI/CD GitHub Actions** build et pousse l'image sur GHCR ; **Terraform + Kubernetes** (Oracle Always-Free) pour la cible de production ; **démo live sur Hugging Face Spaces** (gratuit, sans carte bancaire).
📦 Déploiement one-command (prod) : `make deploy`.
...
🌐 Démo en ligne (Hugging Face Spaces) : [URL_A_RENSEIGNER_APRES_DEPLOIEMENT]
