...
#
# Foundation a valider : necessite un compte OCI Always-Free et les variables
# ci-dessous (tenancy/user/compartment OCID, fingerprint, cle SSH). Le deploiment
# applicatif (k8s/*.yaml) est applique via cloud-init qui installe K3s puis
# `kubectl apply -f k8s/` depuis la branche main.
#
# Usage:
#   terraform init
#   terraform apply \
...
variable "region" { type = string, default = "eu-marseille-1" }
variable "instance_shape" { type = string, default = "VM.Standard.A1.Flex" }
variable "instance_ocpus" { type = number, default = 4 }
variable "instance_memory" { type = number, default = 24 }
variable "repo" { type = string, default = "Sow221/cif-credit-intelligence" }
